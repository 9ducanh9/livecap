...
resource "aws_ecs_task_definition" "preview_backend" {
  ...
  container_definitions = jsonencode(concat([{
    ...
    environment = [
      ...
      { name = "ENABLE_ROOM_SCREEN_SHARE", value = tostring(var.preview_enable_room_screen_share) },
      { name = "IVS_REALTIME_REGION", value = "us-east-1" },
      { name = "IVS_PARTICIPANT_TOKEN_SECONDS", value = tostring(var.room_live_ttl_seconds) },
      { name = "ROOM_TABLE_NAME", value = try(aws_dynamodb_table.room_events[0].name, "") },
      { name = "ROOM_TTL_SECONDS", value = tostring(var.room_live_ttl_seconds) },
      { name = "ROOM_RETENTION_DAYS", value = tostring(var.room_retention_days) },
      { name = "ROOM_MAX_SEGMENTS", value = tostring(var.room_max_segments) },
